...
)/(4.*pow(rh,2)))) + 2*(1 - rgrid[i])*Qr1(i,j,k)*
      (-8*(1 - rgrid[i]) + (6*(1 - pow(1 - rgrid[i],2))*
           (1 - rgrid[i])*Qrr(i,j,k))/
         (1 + (1 - pow(1 - rgrid[i],2))*Qrr(i,j,k)) + 
...
+ Qtr(i,j,k),2))/(4.*pow(rh,6)) - 
        (pow(1 - pow(1 - rgrid[i],2),2)*pow(-1 + rgrid[i],2)*
         ...
         (4.*pow(rh,2))) + 4*Qrr.diff(d010,i,j,k)*
      (1 - pow(1 - rgrid[i],2))*pow(1 - rgrid[i],2)*
      ((-3*pow(1 - pow(1 - rgrid[i],2),2)*pow(Qr1(i,j,k),2))/
         (1 + (1 - pow(1 - rgrid[i],2))*Qrr(i,j,k)) - 
        (8*pow(rh,2)*sqrt(1 + pow(R0,2) + 
