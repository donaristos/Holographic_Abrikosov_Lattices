...
#pragma omp parallel for
    ...
    ReadFromBin(Q, index, 16, tnp, tnx, tny, "tempdata");
    
    for (My_Int i=0; i<16; ++i) {
        Q[i].update(D);
    }
	
	My_Int NN(GetLength<MyType>(9, "psmesh.dat"));
	...
	{My_Int nn(0); MyType w1=0, w2=0, w3=0, w4=0, tw2=0, tw3=0,tw4=0;
		while (nn<NN) {
        ps=psArray+nn*9;
            if(rank==0){
		std::cout << "==========" << std::endl;
        ...
        {   tw2=w2;
            ...
            std::cout << "ksi= " << w3 << std::endl;
                ...
                WriteToBin(Q, index, 16, tnp, tnx, tny, "tempdata");}}
		while( fabs(1-(tw2/w2))>.2 || fabs(1-(tw3/w3))> .2 || fabs(1-((1+tw4)/(1+w4)))> .2);
	
//			if (w2<5.E-5) {
            if(rank==0){
	AppendToBin(Q, index, 16, tnp, tnx,tny, "thermodata");
    WriteArray(psArray+(nn+1)*9,9*(NN-nn-1),"psmesh.dat");
    AppendArray(psArray+9*nn,9,"psstamp.dat");}
//			}
			++nn;
	}
	}
